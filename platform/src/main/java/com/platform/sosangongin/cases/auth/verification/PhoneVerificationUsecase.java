...
public class PhoneVerificationUsecase {
    ...
    public PhoneVerificationResult handlePhoneVerification(PhoneVerificationRequest req) {
        ...
        log.info("phone number verification process begin for user {}", req.getUserId());
        ...
        if(user.isPhoneVerified()){
            ...
            PhoneVerification phoneVerification = PhoneVerification.builder()
                    .code(this.randomCharGeneratorService.getRandomNumber(5, UUID.randomUUID().toString()))
    ...
    public PhoneVerificationResult verifyCodeFromUser(PhoneVerificationRequest req) {
        log.info("phone number verification process begin for user {}", req.getUserId());
        Optional<User> userInfo = this.userRepository.findById(UUID.fromString(req.getUserId()));
        ...
        if (user.isPhoneVerified()) {
            ...
            Optional<PhoneVerification> topByUserOrderByCreatedAtDesc = this.phoneVerificationRepository.findTopByUserOrderByCreatedAtDesc(user);
            if (topByUserOrderByCreatedAtDesc.isEmpty()) {
            ...
            PhoneVerification phoneVerification = topByUserOrderByCreatedAtDesc.get();
