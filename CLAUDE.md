...
This is a **Gradle monorepo** for a platform backend with three main modules:
- `_platform` — Core platform API (Spring Boot 3.2.1, Java 17)
- `_backoffice/backend` — Admin API (Spring Boot, minimal bootstrap)
- `_backoffice/frontend` — Admin UI (Next.js 14, TypeScript)
...
services/    ← External integrations (JWT, OAuth/Kakao, SMS push)
domains/     ← JPA entities + Spring Data repositories
config/      ← Spring configuration (Swagger/OpenAPI)
errors/      ← Custom exceptions
```

The flow is: **Use Case** → **Service/Repository** → **Domain Entity**. Use cases coordinate one business flow end-to-end; services handle infrastructure concerns.
...
- **Dual-token JWT**: `accessToken` (15 min) + `refreshToken` (7 days)
...
SMS is mocked locally via `MockSmsPushService`.
